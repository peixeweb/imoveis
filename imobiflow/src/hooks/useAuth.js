  ...
  const handleSignupSolo = useCallback(async ({ nome, creci, whatsapp, email, password }) => {
    ...
    if (authErr) {
      if (authErr.message.includes('already registered') || authErr.message.includes('ja existe')) {
        ...
        if (signInErr) {
          setAuthError('Este email já possui cadastro, mas a senha está incorreta. Tente fazer login.');
          setAuthLoading(false);
          return false;
        }
        const { data: existingProfile } = await supabase.from('corretores').select('*').eq('user_id', signInData.user.id).single();
        if (existingProfile) {
          setCurrentUser(signInData.user);
          setCorretorProfile(existingProfile);
          setAppState('app');
          setAuthLoading(false);
          return true;
        }
  ...
  const handleSignupTeam = useCallback(async ({ equipeNome, nome, creci, whatsapp, email, password }) => {
    ...
    if (authErr) {
      if (authErr.message.includes('already registered') || authErr.message.includes('ja existe')) {
        ...
        const { data: equipeData, error: equipeErr } = await supabase.from('equipes').insert({ nome: equipeNome, admin_user_id: signInData.user.id }).select().single();
